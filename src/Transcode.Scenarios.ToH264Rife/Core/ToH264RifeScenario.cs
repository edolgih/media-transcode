...
public sealed class ToH264RifeScenario : TranscodeScenario
{
	...
	*/
	...
	internal ToH264RifeDecision BuildDecision(SourceVideo video)
	{
		...
		var interpolationModelName =
			ToH264RifeRequest.ResolveInterpolationModelName(Request.InterpolationQualityProfile);
	...
	*/
	...
	/// </summary>
	private TargetContainer ResolveTargetContainer(SourceVideo video)
	{
		if (Request.OutputContainer is not null)
		{
			return Request.OutputContainer;
		}

		var sourceContainer =
			TargetContainer.ParseOptional(video.FileExtension.TrimStart('.'), nameof(video.FileExtension));
		return sourceContainer ?? TargetContainer.Mp4;
	}
	...
	*/
	...
	/// </summary>
	private static string FormatInterpolationFileName(string fileNameWithoutExtension,
		int userFacingTargetFramesPerSecond)
	{

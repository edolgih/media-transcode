...
CLI получает пути к бинарникам из стандартных источников host configuration, например `appsettings.json` и переменных окружения. `Scenarios:ToH264Rife:RifeNcnnPath` требуется только для `toh264rife` и по умолчанию указывает на repo-local layout в `tools/third_party`. Минимальный `appsettings.json` выглядит так:

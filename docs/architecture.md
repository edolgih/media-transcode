...
- ordinary encode, explicit force-encode, and downscale now share the same profile-driven video-settings axis: output-height buckets, content/quality profiles, bucket bounds, and source-bitrate cap adjustment all come from the shared video-settings profile catalog rather than scenario-local hardcoded fallbacks.
...
- ordinary downscale/encode path, including explicit force-encode at source resolution, stays minimal;
